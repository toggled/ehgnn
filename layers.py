...
def masked_inverse(values: Tensor, exponent: float = 1.0) -> Tensor:
    """Invert positive forward degrees while keeping zero-degree gradients finite."""
    active = (values.detach() > 0).to(values.dtype)
    safe_values = values.clamp_min(torch.finfo(values.dtype).eps)
    return active * safe_values.pow(-exponent)
...
#         zeros(self.bias)
    ...
    def forward(self, x, edge_index: Adj,
                size: Size = None, return_attention_weights=None, norm=None):
...
#         ipdb.set_trace()
        out = self.propagate(edge_index, x=x_V,
                             alpha=alpha_r, norm=norm, aggr=self.aggr)
...
#         This is because O = softmax(QK^T)V and V = V_in*W^V. So W^O can be effectively taken care by W^V!!!
    ...
    def message(self, x_j, alpha_j,
        ...
        message_weight = 1.0 if norm is None else norm.view(-1, 1, 1)
        return x_j * alpha.unsqueeze(-1) * message_weight
...
class HypergraphConvOld(MessagePassing):
    ...
    def forward(self, x: Tensor, hyperedge_index: Tensor,
                ...
                fixed_self_loops: bool = False) -> Tensor:
        r"""
        Args:
        ...
        if fixed_self_loops and self.use_attention:
            raise ValueError("Implicit fixed self-loops do not support attention")
...
class HypergraphConv(MessagePassing):
    ...
    def forward(
        ...
        if (hyperedge_index.numel() == 0 or nnz == 0) and not fixed_self_loops:
            out = x.new_zeros((num_nodes, self.heads * self.out_channels if self.concat else self.out_channels))
            if self.bias is not None:
                out = out + self.bias
            return out

        num_edges = int(hyperedge_index[1].max()) + 1 if nnz else 0
        ...
        if not self.symdegnorm:
            ...
            D = masked_inverse(D, exponent=0.5)
    ...
    def message(
        ...
        scale = norm_i * inc_weight
        if edge_weight is not None:
            scale = scale * edge_weight
        out = scale.view(-1, 1, 1) * x_j.view(-1, H, Fout)
