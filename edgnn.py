...
class EquivSetGNN(nn.Module):
    def __init__(self, num_features, num_classes, args):
        ...
        self.classifier = MLP(in_channels=args.MLP_hidden,
            ...
            InputNorm=False)
        if args.mode == 'learnmask':              # EHGNN-F
            self.mask_module = LearnableEdgeMask(
                num_incidences=int(args.num_incidences), args=args
            )
        elif args.mode == 'learnmask_cond':         # EHGNN-F(cond)
        ...
        elif args.mode == 'learnmask+':             # EHGNN-C(cond)
            self.mask_module = LearnableEdgeMaskplus(args.F,hidden_dim=args.coarse_MLP, args=args)
        elif args.mode == 'learnmask+_agn':         # EHGNN-C
            self.mask_module = FeatureAgnosticEdgeMask_nob(
                num_edges=int(args.num_hyperedges),
                args=args,
            )
        elif args.mode == 'Neural':                 # EHGNN-C(cond,LR)
            self.mask_module = EHGNN_CcondLR(
                args=args, rank=4, 
            )
        elif args.mode == 'NeuralF':                # EHGNN-F(cond,LR)
            self.mask_module = EHGNN_FcondLR(
                args=args, rank=4, 
            )  
    def reset_parameters(self):
